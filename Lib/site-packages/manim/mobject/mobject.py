...
class Mobject:
    ...
    def align_on_at(
            self,
            mobject_or_point: Union["Mobject", np.ndarray, List],
            on = ORIGIN, #direction of the point to align/move
            at = ORIGIN, #direction on static object of where that point should go
            buffer = 0,
        ):
            """Matches 'on' critical point of mobject to  'at' of another
            :class:`~.Mobject`, and adds buffer between them
            """


            if isinstance(mobject_or_point, Mobject):
                shift_vect = -self.get_critical_point(on) + mobject_or_point.get_critical_point(at) + buffer
            else:
                shift_vect = -self.get_critical_point(on) + mobject_or_point + buffer
